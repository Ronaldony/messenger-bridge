...
- direction and exact destination, when known;
...
Default to one-shot delivery, text payload, and the strongest evidence available. Do not infer an exact destination, account authority, or persistent operation. Do not ask whether the user wants MCP; MCP is one possible implementation.
...
Ask at most one compact group of related questions at a time. Do not ask for information that can be discovered from connected tools or local configuration. When a choice has a clear low-risk default, recommend it and ask only for approval to proceed.

## Authorization and secret handling

- Research and read-only discovery may proceed without installation approval.
- Obtain approval before installing software, changing host configuration, starting a persistent service, authenticating an account, creating a bot or webhook, changing channel permissions, or sending a live test.
...
3. Confirm the exact source and destination can be resolved.
4. Confirm the required capabilities and payload limits from actual schemas or probes.
5. Prefer a non-sending dry run when the adapter provides one.
6. After explicit live-send authorization, execute one marker-bearing test and report the achieved assurance level.
